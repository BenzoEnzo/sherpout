...
class TrainingPlanService {
  ...
  Future<List<TrainingPlanDTO>> getAll() async {
    Response<dynamic> response = await _apiClient.get('training-plans');
    final List<dynamic> data = response.data;
    return data.map((json) => TrainingPlanDTO.fromJson(json)).toList();
  }
  ...
  Future<TrainingPlanDTO> create(TrainingPlanDTO trainingPlan) async {
    Response<dynamic> response =
        await _apiClient.post('training-plans', data: trainingPlan.toJson());
    final dynamic data = response.data;
    return TrainingPlanDTO.fromJson(data);
  }

  Future<TrainingPlanDTO> update(TrainingPlanDTO record) async {
    Response<dynamic> response = await _apiClient
        .put('training-plans/${record.id}', data: record.toJson());
    final dynamic data = response.data;
    return TrainingPlanDTO.fromJson(data);
  }
}
